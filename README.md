...
**重复导入会被标出来，但不会替你跳过。** 如果库里已经有一份同名同大小的文件，
那一行会带上「可能重复」的标记，顶部也会多出一个「排除 N 个重复项」的按钮。
不点它，这些文件照样导入。

> 为什么默认不跳过：同名同大小也可能是两份都该留的文件（比如同一份课件改了个错字又存回来一份）。
> 提醒是帮忙，替你做主丢掉一份就不一样了——丢的是一份真文件。所以只标出来，跳过与否你点一下决定。
...
**还原之前会先验一下包。** 选错了文件（比如把「导出全部资料」的包当成备份），
或者下载没下完的残包，都会在动手清空之前被拦下来，现有数据不会有事。

### 让浏览器别自动清理它

默认情况下浏览器把网页数据当缓存看，磁盘紧张时可以直接清掉。设置页里有一个
**「申请持久化存储」** 的按钮，批下来之后，除非你自己动手清理，浏览器不会自动删掉资料。

在 Chrome / Edge 里，把本站「安装到主屏幕 / 桌面」（见下一节）之后再申请，基本都会批。

移动端和电脑之间搬资料，走的也是这个流程：**电脑上导出 zip → 传过去 → 手机上导入**。

---

## 直接导出成一棵文件夹树（Chrome / Edge 桌面版）

如果比起 zip，你更想直接看到一个能翻的文件夹，设置页里还有一个
**「导出到文件夹…」**：点一下、挑一个文件夹，资料就按「学期 / 课程 / 分类」
原样写进去，文件名也不改，用资源管理器或者别的软件都能直接打开。
...
`_` 开头的 json 是给程序看的说明文件，别删，其余就是你的资料。

**这是搬运，不是同步。** 导完之后你在网站里再改什么，那个文件夹不会跟着变，
想跟上就再导一次。它也不会删东西——上次导出留下的、这次资料库里已经没有的文件，
只会在报告里列出来告诉你，删不删由你决定。

只肯往**空文件夹**或者**上次导出的那个文件夹**里写。挑了一个装着别的东西的文件夹会被挡住，
免得把你的照片、文档和课程文件混在一起。

写出去之前会检查名字：Windows 不允许的字符换成 `_`，`CON` / `LPT1` 这类设备名加前缀，
太长的截短（保留扩展名），重名的加 `(2)`。凡是动过名字的都会在最后的报告里列出来——
不会静默改名，因为你是照着自己记的名字去找文件的。

**Firefox、Safari 和手机浏览器用不了这个功能**（它们不给网页写文件夹的权限），
按钮不会显示，会提示你用上面的导出 zip，解压出来是一样的结构。
...
- **不自动去重。** 同一份文件导入两次会出现两条记录。预览里会标出「可能重复」并提供一键排除，
  但**默认不替你跳过**——同名同大小也可能是两份都该留的文件。
- **不支持自动读微信/QQ 的文件目录。** 浏览器没有权限读本机文件系统，而且微信的 `.dat`
  文件是加密的。老老实实用「选文件夹导入」。
- **建议总数据量控制在 2GB 以内。** 浏览器给网页的存储配额有限，超了会被拒绝写入。
  设置页的「存储空间」里能看到真实的额度和已用量——那个数会比文件大小加出来的大，
  因为索引和提取出的正文也占地方。
...
node scripts/test-folder-export.mjs # 导出到文件夹的路径规划单元测试
...
建学期 → 建课程 → 导入 → 归类 → 入库 → 提取 → 搜索 → 导出 → 导出到文件夹
→ 备份 → 还原的全流程，中间断开网络重载一次确认离线可用，
并断言全程零未捕获异常。它需要本机装有 Chrome，也可以用 `CHROME_PATH` 指定。

几条专门为「用户会因此丢数据」的路径写的用例：

- **备份 → 清空 → 还原的完整往返**，还原后正文要能被重新提取、搜索恢复可用；
- **两个坏包必须被拒收**（不是 zip 的、以及 metadata 里登记的文件大半不在包里的），
  而且拒收之后原数据必须一条不少；
- **写满存储时**（用故障注入造出 `QuotaExceededError`）要给出明确提示、进度框不能挂死、
  写不进去的文件不能留下半条记录；
- **导出到文件夹**时核对磁盘上的树和库里的数据对不对得上（文件名集合、每个文件的字节数、
  侧车数量），并确认往一个装着别人文件的文件夹里导会一个字节都不写。
